...
void update_environment(char* base_path)
{
    /*
     ...
     1) PIN_VM32_LD_LIBRARY_PATH and PIN_VM64_LD_LIBRARY_PATH (put pin_libs in front of existing LD_LIBRARY_PATH)
     ...
     Pin's (Bionic-based) loader can't handle a library with the wrong bitness on
    ...
    char* base_path32 = 0;
    char* base_path64 = 0;
    const char* pin_runtime_dir = "runtime/pincrt";
    ...
    xed_runtime_libs32 = append3(extras_path, "/", "xed-ia32/lib");
    xed_runtime_libs64 = append3(extras_path, "/", "xed-intel64/lib");
    ...
    pin_32_ld_library_path = append3(pin_runtime_libs32, ":", ld_library_path);
    pin_64_ld_library_path = append3(pin_runtime_libs64, ":", ld_library_path);
    ...
    new_32_ld_library_path = append3(pin_runtime_libs32, ":", ld_library_path);
    new_64_ld_library_path = append3(pin_runtime_libs64, ":", ld_library_path);

    /* must be first (so added last) */
    new_32_ld_library_path = append3(xed_runtime_libs32, ":", new_32_ld_library_path);
    new_64_ld_library_path = append3(xed_runtime_libs64, ":", new_64_ld_library_path);
...
int check_file_exist(const char* fname)
{
    ...
    return OS_RETURN_CODE_IS_SUCCESS(OS_GetFileAttributes(fname, &attr)) && (attr & OS_FILE_ATTRIBUTES_EXIST);
}
