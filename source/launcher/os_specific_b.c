...
char* find_driver_name(char* argv0)
{
    ...
    proc_link = "/proc/curproc/file";
    if (check_file_exists(proc_link) == 0)
    {
        fprintf(stderr,
                "PIN ERROR: Pin needs /proc mounted for proper operation\n"
                "\tAs root: mount -t procfs proc /proc\n"
                "\tAnd add  \"proc /proc procfs rw 0 0\" "
                "(no quotes) to /etc/fstab\n\n");
        exit(1);
    }
        ...
        char** user_argv)
{
    ...
    /*
     * Pin currently have support only for 64 bit BSD versions.
     */
